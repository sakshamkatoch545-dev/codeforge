...
export const Navbar: React.FC<NavbarProps> = ({ currentUser, onLogout }) => {
  ...
  return (
    <div className="sticky top-6 z-50 px-6 animate-fade-in">
      <header className={`h-16 max-w-6xl mx-auto glass-panel !rounded-full flex items-center px-8 justify-between shadow-2xl ${theme.shadow} border ${theme.border} backdrop-saturate-200 transition-all duration-500`}>
        <Link to="/" className={`text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r ${theme.logoGradient} tracking-tight flex items-center gap-3 uppercase group transition-colors duration-500`}>
          <div className={`relative flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-tr ${theme.logoIconBg} shadow-lg group-hover:scale-110 transition-all duration-300`}>
        ...
        <nav className="space-x-8 flex items-center">
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors text-gray-700 dark:text-gray-200 flex items-center justify-center cursor-pointer"
